...
class LoginViewController: UIViewController, TextFieldNavigationDelegate{
    ...
    @IBAction func loginButtonClicked(_ sender: Any) {
        if let attractionsListViewController = self.storyboard?.instantiateViewController(withIdentifier: "touristAttractionsViewControllerId") as? TouristAttractionsViewController {
            self.navigationController?.pushViewController(attractionsListViewController, animated: true)
        }
    }
